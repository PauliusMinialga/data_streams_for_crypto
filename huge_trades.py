...
symbols = ['btcusdt', 'ethusdt', 'bnbusdt', 'solusdt', 'dogeusdt', 'wifusdt']
...
class TradeAggregator:
    ...
    async def check_and_print_trade(self):
        ...
        for trade_key, usd_size in self.trade_buckets.items():
            ...
            if usd_size > 500000: #the comparison between second and timestamp_now was giving issues, and without it code runs fine
                attrs = ['bold']
    ...
    async with connect(uri) as websocket:
        while True:
            try:
                ...
                trade_time = datetime.fromtimestamp(int(data['T'])/1000, pytz.timezone('Europe/Amsterdam'))
...
async def main():
    ...
    trade_stream_tasks = [binance_trade_stream(f"{websocket_url}{symbol}@aggTrade", symbol, filename, trade_aggregator) for symbol in symbols]
